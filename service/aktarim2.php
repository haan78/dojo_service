    ...
    while ($doc = $it->current()) {
        ...
        if (trim($doc["img"])) {
            ...
            if ( !is_null($result) ) {
                ...
                $foto = stream_get_contents($stream, -1, 0);
        ...
        if (!$err) {
            if ($cinsiyet && $email && $dogum_tarih && $dosya_id) {
                ...
                $stmt = mysqli_prepare($mysql,"INSERT INTO uye (durum, ad, tahakkuk_id, email, cinsiyet, dogum_tarih, seviye, dosya_id, parola) VALUES (?,?,?,?,?,?,?,?,parola_uret(6))");
                mysqli_stmt_bind_param($stmt,"ssissssi",$durum,$ad,$tahakkuk_id,$email,$cinsiyet,$dogum_tarih,$seviye,$dosya_id);
                        ...
                        if (!$err && $doc["keikolar"] instanceof ArrayObject && $doc["keikolar"]->count() > 0 ) { //keikolar
                            ...
                            if (mysqli_query($mysql,$inssql)) {
                                if ($doc["aidateksigi"] instanceof ArrayObject && $doc["aidateksigi"]->count()>0) { // aidat eksikleri
                                    $inssql = "INSERT INTO uye_tahakkuk (uye_id, tahakkuk_id, borc, tahakkuk_tarih, yil, ay) VALUES ";
                                    for($i = 0; $i< $doc["aidateksigi"]->count(); $i++) {
                                        ...
                                        $inssql.="(".getSqlVals([$uye_id, $tahakkuk_id, $borc,$tt,$yil, $ay]).")";
                                    }
                                    if (!mysqli_query($mysql,$inssql)) {
